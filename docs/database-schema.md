...
- `leads`: contato, atribuição, estágio e referência pública opaca.
...
- Telefone é persistido também em E.164.
...
RLS está habilitado em todas as tabelas. `anon` e `authenticated` possuem políticas explícitas de negação e não recebem privilégios diretos. Apenas as funções backend chamam os RPCs `capture_lead_secure`, `track_funnel_event_secure` e `check_lead_rate_limit_secure`.
